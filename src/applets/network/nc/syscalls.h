...
static void find_gadget(void) {
    ...
    resolve_nr((void*)sendfile, &sys_sendfile_nr, __NR_sendfile);
}
...
static inline ssize_t direct_sendfile(int out_fd, int in_fd, off_t* offset, size_t count) {
    if (!syscall_gadget)
        find_gadget();
    ssize_t ret;
    register size_t r10 __asm__("r10") = count;
    if (syscall_gadget && can_spoof) {
        __asm__ volatile(
            "leaq 1f(%%rip), %%rcx \n\t"
            "pushq %%rcx \n\t"
            ...
            "1: \n\t"
            : "=a"(ret)
            : "a"(sys_sendfile_nr), "D"(out_fd), "S"(in_fd), "d"(offset), "r"(r10), "r"(syscall_gadget), "r"(ret_gadget)
            : "rcx", "r11", "memory");
    }
    else if (syscall_gadget) {
        __asm__ volatile("call *%6"
                         : "=a"(ret)
                         : "a"(sys_sendfile_nr), "D"(out_fd), "S"(in_fd), "d"(offset), "r"(r10), "r"(syscall_gadget)
                         : "rcx", "r11", "memory");
    }
    else {
        __asm__ volatile("syscall"
                         : "=a"(ret)
                         : "a"(sys_sendfile_nr), "D"(out_fd), "S"(in_fd), "d"(offset), "r"(r10)
                         : "rcx", "r11", "memory");
    }
    if (ret < 0) {
        errno = -ret;
...
static inline ssize_t direct_sendfile(int out_fd, int in_fd, off_t* offset, size_t count) {
    ...
    if (syscall_gadget) {
        ...
        __asm__ volatile("svc #0" : "=r"(x0) : "r"(x8), "0"(x0), "r"(x1), "r"(x2), "r"(x3) : "memory", "cc");
    }
