...
static bool bx_copy_should_follow_source(const struct bx_copy_options *options,
                                         bool top_level,
...
static bool bx_copy_apply_path_attrs(const struct bx_copy_context *ctx,
                                     const char *src_path,
                                     const char *dest_path,
                                     const struct stat *src_stat,
                                     bool no_follow,
...
static void bx_copy_add_link_entry(struct bx_copy_context *ctx, const struct stat *st, const char *dest_path) {
    if ((ctx->options->preserve_mask & BX_PRESERVE_LINKS) == 0 &&
...
static void bx_copy_diag_self_recursive_copy(struct bx_copy_context *ctx,
                                             const char *src_path,
                                             const char *dest_path) {
    ...
    if (dest_state.exists_stat && bx_same_file(src_stat, &dest_state.st)) {
        if (ctx->backup_params.mode != BX_BACKUP_NONE && ctx->options->force &&
            ...
            enum bx_backup_create_result backup_result =
                bx_copy_backup_same_file_copy(ctx, src_path, dest_path);
...
static bool bx_copy_regular_file_path(struct bx_copy_context *ctx,
                                      const char *src_path,
                                      const char *dest_path,
                                      const struct stat *src_stat) {
...
static bool bx_copy_fifo_contents(struct bx_copy_context *ctx,
                                  const char *src_path,
                                  const char *dest_path,
                                  const struct stat *src_stat) {
...
static bool bx_copy_create_fifo_node(struct bx_copy_context *ctx,
                                     const char *dest_path,
                                     mode_t create_mode) {
...
static bool bx_copy_create_socket_node(struct bx_copy_context *ctx,
                                       const char *dest_path,
                                       mode_t create_mode) {
...
static bool bx_copy_special_node(struct bx_copy_context *ctx,
                                 ...
                                 const char *dest_path,
                                 const struct stat *src_stat,
                                 bool (*create_node)(struct bx_copy_context *ctx,
                                                     const char *dest_path,
                                                     mode_t create_mode)) {
    struct bx_dest_state dest_state;
    mode_t create_mode = bx_copy_regular_file_create_mode(ctx, src_stat);
...
static bool bx_copy_create_device_node(struct bx_copy_context *ctx,
                                       const char *dest_path,
                                       mode_t create_mode,
                                       dev_t rdev) {
...
static bool bx_copy_device_node(struct bx_copy_context *ctx,
                                 const char *src_path,
                                 const char *dest_path,
                                 const struct stat *src_stat) {
    struct bx_dest_state dest_state;
...
static bool bx_copy_fifo(struct bx_copy_context *ctx,
                          const char *src_path,
                          const char *dest_path,
                          const struct stat *src_stat) {
...
static bool bx_copy_socket_contents(struct bx_copy_context *ctx,
                                     const char *src_path,
                                     const char *dest_path,
                                     const struct stat *src_stat) {
    return bx_copy_regular_file(ctx, src_path, dest_path, src_stat, true);
...
static bool bx_copy_socket(struct bx_copy_context *ctx,
                            const char *src_path,
                            const char *dest_path,
                            const struct stat *src_stat) {
...
static bool bx_copy_symlink_object(struct bx_copy_context *ctx,
                                    const char *src_path,
                                    const char *dest_path,
...
static bool bx_copy_create_symbolic_link(struct bx_copy_context *ctx,
...
static bool bx_copy_create_hard_link(struct bx_copy_context *ctx,
                                     const char *src_path,
                                     const char *dest_path,
                                     const struct stat *src_stat,
                                     bool follow_source) {
...
static bool bx_copy_directory(struct bx_copy_context *ctx,
                              const char *src_path,
                              const char *dest_path,
                              const struct stat *src_stat,
                              bool top_level) {
...
bool bx_copy_path(struct bx_copy_context *ctx,
                  const char *src_path,
                  const char *source_operand,
                  const char *dest_path,
                  bool top_level) {
    ...
    if (follow_source &&
        ...
        bx_copy_source_dir_in_stack(ctx, src_stat.st_dev, src_stat.st_ino)) {
        bx_copy_diag_cyclic_symlink(ctx, src_path);
        return false;
    }

    if (ctx->options->parents && !bx_copy_prepare_parents(ctx, source_operand)) {
        return false;
    }
    ...
    if (S_ISDIR(src_stat.st_mode)) {
        ...
        return bx_copy_directory(ctx, src_path, dest_path, &src_stat, top_level);
    }

    if (ctx->options->symbolic_link) {
        return bx_copy_create_symbolic_link(ctx, source_operand, dest_path, &src_stat);
    }
    if (ctx->options->hard_link) {
        return bx_copy_create_hard_link(ctx, src_path, dest_path, &src_stat, follow_source);
    }
    ...
    if ((ctx->options->preserve_mask & BX_PRESERVE_LINKS) != 0u &&
        ...
        if (entry != NULL) {
            return bx_copy_create_hard_link(ctx, entry->dest_path, dest_path, &src_stat, false);
        }
    }

    if (!follow_source && S_ISLNK(src_lstat.st_mode)) {
        return bx_copy_symlink_object(ctx, src_path, dest_path, &src_lstat);
    }
    if (S_ISFIFO(src_stat.st_mode)) {
        if (!ctx->options->recursive || ctx->options->copy_contents) {
            return bx_copy_fifo_contents(ctx, src_path, dest_path, &src_stat);
        }
        return bx_copy_fifo(ctx, src_path, dest_path, &src_stat);
    }
    if (S_ISSOCK(src_stat.st_mode)) {
        if (!ctx->options->recursive || ctx->options->copy_contents) {
            return bx_copy_socket_contents(ctx, src_path, dest_path, &src_stat);
        }
        return bx_copy_socket(ctx, src_path, dest_path, &src_stat);
    }
    if (S_ISCHR(src_stat.st_mode) || S_ISBLK(src_stat.st_mode)) {
        if (ctx->options->copy_contents) {
            return bx_copy_regular_file_path(ctx, src_path, dest_path, &src_stat);
        }
        return bx_copy_device_node(ctx, src_path, dest_path, &src_stat);
    }
