...
static bool int_view_from_value(const struct expr_value *value,
...
static int compare_values(const struct expr_value *a, const struct expr_value *b) {
    ...
    if (int_view_from_value(a, &av, a_buf, sizeof(a_buf))
        && int_view_from_value(b, &bv, b_buf, sizeof(b_buf))) {
...
static void digits_divmod_abs(const char *numerator,
...
static bool bigint_divmod(struct bigint *quotient,
                          struct bigint *remainder,
                          const struct bigint *a,
...
static bool value_to_bigint_required(const struct expr_value *value,
                                     struct expr_eval_ctx *ctx,
                                     struct bigint *out) {
    if (value->type == VALUE_INT) {
        char buf[64];
...
static bool eval_regex_match(const char *input,
                             const char *pattern,
                             struct expr_eval_ctx *ctx,
    ...
    if (re.re_nsub > 0) {
        ...
        size_t end = (whole.rm_eo < 0 || whole.rm_eo < whole.rm_so)
            ? start
            : (size_t)whole.rm_eo;
...
static bool eval_binary_op(const struct expr_node *node, struct expr_eval_ctx *ctx, struct expr_value *out) {
    ...
    case OP_MOD: {
        ...
        if (!value_to_bigint_required(&left, ctx, &lnum)
            || !value_to_bigint_required(&right, ctx, &rnum)) {
            bigint_free(&lnum);
            bigint_free(&rnum);
            ok = false;
            break;
...
int bx_expr_main(int argc, char **argv) {
    ...
    if (root && parser_peek(&parser) && parser_peek(&parser)->kind != TOK_END) {
        parser_set_error(&parser,
                         ...
                         parser_peek(&parser)->text ? parser_peek(&parser)->text : "");
        node_free(root);
        root = NULL;
    }
