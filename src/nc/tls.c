...
void tls_setup_client(struct tls* tls_ctx, int s, char* host) {
    ...
    if (tls_connect_socket(tls_ctx, s, tls_expectname ? tls_expectname : host) == -1) {
        tls_free(tls_ctx);
        close(s);
        errx(EXIT_RUNTIME, "tls connection failed (%s)", tls_error(tls_ctx));
    }
    if (timeout_tls(s, tls_ctx, tls_handshake) == -1) {
        ...
        errx(EXIT_RUNTIME, "tls handshake failed (%s)", errstr);
    }
    if (vflag)
        report_tls(tls_ctx, host, s);
    if (tls_expecthash &&
        (tls_peer_cert_hash(tls_ctx) == NULL || strcmp(tls_expecthash, tls_peer_cert_hash(tls_ctx)) != 0)) {
        tls_free(tls_ctx);
        close(s);
        errx(EXIT_RUNTIME, "peer certificate is not %s", tls_expecthash);
    }
    if (Zflag) {
        ...
        if (Zflag != stderr && (fclose(Zflag) != 0)) {
            tls_free(tls_ctx);
            close(s);
            err(EXIT_RUNTIME, "fclose failed saving peer cert");
        }
    }
}
...
struct tls* tls_setup_server(struct tls* tls_ctx, int connfd, char* host) {
    ...
    if (tls_accept_socket(tls_ctx, &tls_cctx, connfd) == -1) {
        ...
        if (vflag && gotcert)
            report_tls(tls_cctx, host, connfd);
...
int process_tls_opt(char* s, int* flags) {
    ...
    for (t = tlskeywords; t->keyword != NULL; t++) {
        if (strlen(t->keyword) == len && strncmp(s, t->keyword, len) == 0) {
            if (t->value != NULL) {
                if (v == NULL)
                    errx(EXIT_USAGE, "invalid tls value `%s'", s);
                *t->value = v;
            }
            else {
                if (v != NULL)
...
void save_peer_cert(struct tls* tls_ctx, FILE* fp) {
    ...
    if ((pem = (const char*)tls_peer_cert_chain_pem(tls_ctx, &plen)) == NULL)
        errx(EXIT_RUNTIME, "Can't get peer certificate");
    if (fprintf(fp, "%.*s", (int)plen, pem) < 0)
        err(EXIT_RUNTIME, "unable to save peer cert");
    if (fflush(fp) != 0)
        err(EXIT_RUNTIME, "unable to flush peer cert");
}
...
void report_tls(struct tls* tls_ctx, char* host, int fd) {
    ...
    if (jflag) {
        ...
        const char* direction = lflag ? "in" : "out";
        const char* protocol = uflag ? "udp" : "tcp";

        if (json_socket_tuple_from_fd(fd, src_addr, sizeof(src_addr), src_port, sizeof(src_port), dst_addr,
        ...
        json_event_begin(stderr, "info", "tls_handshake_succeeded", direction, protocol, "established", "disabled",
                         src_addr_p, src_port_p, dst_addr_p, dst_port_p);
        fprintf(stderr, ",\"host\":\"%s\",\"version\":\"%s\",\"cipher\":\"%s\"", host, tls_conn_version(tls_ctx),
