...
void
mparse_readmem(struct mparse *curp, const char *buf, size_t sz,
    const char *filename)
{
	struct buf	 blk;
	struct buf	*save_primary;
	...
	if ((cp = filename_sec(filename)) != NULL &&
	    cp[1] >= '1' && cp[1] <= '9')
		curp->man->filesec = cp[1];
	else
		curp->man->filesec = '\0';

	blk.buf = mandoc_malloc(sz + 1);
	memcpy(blk.buf, buf, sz);
	blk.buf[sz] = '\0';
	blk.sz = sz;
	blk.next = NULL;
...
mparse_open(struct mparse *curp, const char *file)
{
	 ...
	 * already end in a known compressed suffix,
...
const struct buf *
mparse_getsecondary(const struct mparse *p)
{
