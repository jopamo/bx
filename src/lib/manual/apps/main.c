 ...
 *
 * Main program for mandoc(1), man(1), and help(1).
 */
#include "config.h"
...
static	int		  fs_search(const struct mansearch *,
				...
				struct manpage **, size_t *);
static	void		  free_manpages(struct manpage *, size_t);
static	void		  glob_esc(char **, const char *, const char *);
static	char		 *load_plain_input(int, size_t *);
...
main(int argc, char *argv[])
{
	 ...
	 * using it as a search term for man(1).
	 */

	if (conf.output.tag != NULL && *conf.output.tag == '\0') {
		tagarg = argc > 0 ? *argv : NULL;
		conf.output.tag = tagarg;
	}
	...
	if (search.argmode == ARG_FILE && auxpaths != NULL) {
		if (strcmp(auxpaths, "doc") == 0)
			...
			options |= MPARSE_MAN;
		else if (strcmp(auxpaths, "markdown") == 0)
			intype = INTYPE_MARKDOWN;
	}
	...
	for (i = 0; i < ressz; i++) {
		if (i > 0)
			mparse_reset(mp);
		process_onefile(mp, res + i, &outst, &conf, intype);
...
usage(enum argmode argmode)
{
	...
	case ARG_FILE:
		fputs("usage: mandoc [-ac] [-I os=name] "
		    "[-K encoding] [-mdoc | -man | -mmarkdown] "
		    "[-O options]\n"
		    "\t      [-T output] [-W level] [file ...]\n", stderr);
		break;
	case ARG_NAME:
		fputs("usage: man [-achlw] [-C file] [-M path] "
		    "[-m path] [-S subsection]\n"
		    "\t   [[-s] section] name ...\n", stderr);
		break;
	default:
...
static void
free_manpages(struct manpage *res, size_t sz)
{
	size_t	 i;
...
process_onefile(struct mparse *mp, struct manpage *resp,
    struct outstate *outst, struct manconf *conf, enum intype intype)
{
	int	 fd;
	...
	if (resp->ipath != SIZE_MAX &&
	    chdir(conf->manpath.paths[resp->ipath]) == -1)
		mandoc_msg(MANDOCERR_OPEN, 0, 0, "%s: %s",
		    conf->manpath.paths[resp->ipath], strerror(errno));
	...
	if (resp->form == FORM_SRC)
		parse(mp, fd, resp->file, outst, conf, intype);
	else {
    ...
    struct outstate *outst, struct manconf *conf, enum intype intype)
{
	...
	if (intype == INTYPE_MARKDOWN) {
		markdown_src = load_plain_input(fd, &markdown_sz);
		if (fd != STDIN_FILENO)
			close(fd);
		if (markdown_src == NULL)
			return;
		lines = NULL;
		markdown_src = man_from_markdown(markdown_src, markdown_sz);
		markdown_sz = strlen(markdown_src);
		mparse_readmem(mp, markdown_src, markdown_sz, file);
		free(markdown_src);
	} else {
...
static char *
load_plain_input(int fd, size_t *sz)
{
	char		*buf;
	size_t		 cap, len;
	ssize_t		 got;
	...
	cap = 4096;
	len = 0;
	buf = mandoc_malloc(cap + 1);
	for (;;) {
		if (len == cap) {
			cap *= 2;
			buf = mandoc_reallocarray(buf, cap + 1, sizeof(*buf));
		}
