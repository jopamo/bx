...
static bool parse_restrict_modes(const EffectiveConfig *cfg,
                                 RestrictFileNameModes *modes_out) {
    if (!modes_out) {
        errno = EINVAL;
        return false;
    }
...
static bool component_needs_encoding(const char *component,
                                     RestrictFileNameModes modes) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (c < 0x20 || c == 0x7f || c == '/' ||
            (modes.windows_mode && is_windows_reserved_char(c))) {
            return true;
        }
    }
    return false;
...
char *pathmap_sanitize_component(const char *component, const EffectiveConfig *cfg) {
    ...
    if (!parse_restrict_modes(cfg, &modes)) return NULL;
...
char *pathmap_canonical_url_to_local(const char *url, const EffectiveConfig *cfg) {
    ...
    if (flatten_layout) {
        ...
        size_t total_len = (proto_part ? strlen(proto_part) + 1 : 0) +
                           (host_part ? strlen(host_part) + 1 : 0) +
        ...
        if (proto_part) { strcat(local_path, proto_part); strcat(local_path, "/"); }
        if (host_part) { strcat(local_path, host_part); strcat(local_path, "/"); }
        if (dir_part && dir_part[0]) { strcat(local_path, dir_part); strcat(local_path, "/"); }
