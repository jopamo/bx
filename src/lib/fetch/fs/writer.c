...
static int writer_fail_temp_entry(int error_number, int parent_fd, int fd,
                                  char **name_inout) {
    if (fd != -1) close(fd);
...
static int capture_parent_directory_identity(Writer *w) {
    ...
    struct stat st;
    if (fstat(w->parent_fd, &st) != 0) return -1;
...
static int validate_parent_directory_identity(const Writer *w) {
     ...
     */
    current_parent_fd = mira_secure_path_open_parent_directory(
        w->path, false, &current_basename);
    ...
    if (rc == 0 &&
        (st.st_dev != w->parent_dev ||
         st.st_ino != w->parent_ino ||
         strcmp(current_basename, w->basename) != 0)) {
        rc = -1;
        error_number = EBUSY;
    }
...
static int open_unique_temp_file_at(int parent_fd, const char *basename, char **temp_name_out) {
    ...
    for (unsigned long long attempt = 0; attempt < 128; attempt++) {
        ...
        if (asprintf(&temp_name, "%s.mira.tmp.%016llx%02llx",
                     basename,
                     serial,
                     attempt) == -1) {
            temp_name = NULL;
        ...
        int fd = mira_secure_path_open_leaf(
            parent_fd, temp_name,
...
static int seed_temp_file_from_existing_destination(Writer *w) {
    ...
    int src_fd = mira_secure_path_open_leaf(
        w->parent_fd, w->basename,
...
static bool same_destination_identity(const Writer *w, const struct stat *st) {
    if (!w || !st || !w->initial_dest_existed) return false;
    return w->initial_dest_dev == st->st_dev &&
           w->initial_dest_ino == st->st_ino &&
           ((w->initial_dest_mode & S_IFMT) == (st->st_mode & S_IFMT));
}
...
static int copy_user_xattrs(int source_fd, int destination_fd) {
    ...
    ssize_t names_length =
        flistxattr(source_fd, names, (size_t)list_size);
    ...
    while (offset < (size_t)names_length) {
        ...
        if (value_size > 0) {
            ...
            ssize_t read_size =
                fgetxattr(source_fd, name, value, (size_t)value_size);
            if (read_size != value_size) {
                int error_number =
                    read_size == -1 ? errno : EIO;
                free(value);
                free(names);
                errno = error_number;
                return -1;
        ...
        if (fsetxattr(destination_fd, name, value,
...
static int copy_metadata(MiraMetadata *dest, const MiraMetadata *src) {
    ...
    MiraMetadata copy = {0};
    if (set_metadata_string(&copy.etag, src->etag) != 0 ||
        set_metadata_string(&copy.last_modified, src->last_modified) != 0 ||
        set_metadata_string(&copy.origin_url, src->origin_url) != 0 ||
        set_metadata_string(&copy.redirect_target, src->redirect_target) != 0 ||
        set_metadata_string(&copy.local_path, src->local_path) != 0) {
        metadata_clear(&copy);
        return -1;
    }
...
static int write_metadata_temp_file_at(int parent_fd, const char *basename,
                                       const MiraMetadata *meta, char **temp_name_out) {
...
static void cleanup_temp_entry(int parent_fd, char **name) {
    if (parent_fd == -1 || !name || !*name) return;
...
static void restore_hold_entry(int parent_fd, char **hold_name, const char *final_name) {
    if (parent_fd == -1 || !hold_name || !*hold_name || !final_name) return;
...
static void writer_free(Writer *w) {
    if (!w) return;
    if (w->parent_fd != -1) close(w->parent_fd);
    free(w->temp_name);
...
static bool parse_backup_index(const char *name, const char *basename, int *index_out) {
    if (!name || !basename || !index_out) return false;
...
static int finalize_payload_hold(int parent_fd, const char *basename, int backups,
                                 bool rotate_backups_on_commit, char **hold_name_inout) {
    if (parent_fd == -1 || !basename || !hold_name_inout) {
        errno = EINVAL;
        return -1;
    }

    if (!*hold_name_inout) return 0;
...
Writer *writer_open_with_options(const char *path, WriterMode mode, int backups, bool unlink_existing) {
    ...
    w->parent_fd =
        mira_secure_path_open_parent_directory(path, true, &w->basename);
    if (w->parent_fd == -1) {
        writer_free(w);
        return NULL;
    }
...
static int writer_set_final_path_with_policy(Writer *w, const char *path,
                                             bool exclusive) {
    ...
    if (!basename_is_simple_leaf(new_basename) ||
        strcmp(new_parent, w->parent_path ? w->parent_path : ".") != 0) {
        free(new_parent);
        free(new_basename);
        errno = EXDEV;
        return -1;
    ...
    if (strcmp(path, w->path) == 0) {
        if (exclusive) {
            ...
            if (ensure_leaf_absent(w->parent_fd, new_basename) != 0 ||
                ensure_leaf_absent(w->parent_fd, sidecar) != 0) {
                int error_number = errno;
                free(sidecar);
                free(new_parent);
                free(new_basename);
    ...
    if (exclusive) {
        ...
        if (ensure_leaf_absent(w->parent_fd, new_basename) != 0 ||
            ensure_leaf_absent(w->parent_fd, new_sidecar) != 0) {
            int error_number = errno;
            free(new_sidecar);
            free(new_parent);
            free(new_basename);
...
int writer_preserve_destination_metadata(Writer *w) {
    ...
    if (!w->initial_dest_existed ||
        !S_ISREG(w->initial_dest_mode)) {
        return 0;
    }

    int source_fd = mira_secure_path_open_leaf(
        w->parent_fd, w->basename,
        O_RDONLY | O_CLOEXEC | O_NOFOLLOW, 0);
...
int writer_write(Writer *w, const void *data, size_t len) {
    if (!w || w->fd == -1 || (!data && len > 0)) return -1;
...
int writer_stage_xattrs(Writer *w, const char *url, const char *content_type,
                        const char *etag, const char *last_modified) {
    if (!w || w->to_stdout || w->fd == -1) {
        errno = EINVAL;
        return MIRA_XATTR_ERROR;
    }
    return mira_xattr_apply_fd(
        w->fd, url, content_type, etag, last_modified);
}
...
static void unlink_leaf_if_same_identity(int parent_fd, const char *name,
                                         const struct stat *expected) {
    if (parent_fd == -1 || !name || !expected) return;

    struct stat current;
    if (fstatat(
            parent_fd, name, &current, AT_SYMLINK_NOFOLLOW) != 0) {
        return;
    }
...
int writer_close(Writer *w) {
    ...
    if (w->has_pending_mtime) {
        ...
        if (futimens(w->fd, times) != 0) {
            ...
            if (w->temp_name) unlinkat(w->parent_fd, w->temp_name, 0);
            writer_free(w);
            return -1;
        }
    }
    ...
    if (w->temp_name) {
        ...
        if (w->has_pending_metadata) {
            ...
            if (!metadata_is_empty(&w->pending_metadata) &&
                write_metadata_temp_file_at(w->parent_fd, sidecar_name, &w->pending_metadata, &sidecar_temp_name) != 0) {
                free(sidecar_name);
                cleanup_temp_entry(w->parent_fd, &w->temp_name);
                writer_free(w);
                return -1;
        ...
        if (!w->exclusive_final_path &&
            rename_existing_entry_to_hold(
                w->parent_fd, w->basename, &payload_hold_name) != 0) {
            free(sidecar_name);
        ...
        if (!w->exclusive_final_path && sidecar_name &&
            rename_existing_entry_to_hold(w->parent_fd, sidecar_name, &sidecar_hold_name) != 0) {
        ...
        if (w->exclusive_final_path && sidecar_name &&
            !sidecar_temp_name &&
            ensure_leaf_absent(w->parent_fd, sidecar_name) != 0) {
            free(sidecar_name);
            cleanup_temp_entry(w->parent_fd, &w->temp_name);
            writer_free(w);
            return -1;
        ...
        if (sidecar_temp_name) {
            if (fstatat(
                    w->parent_fd, sidecar_temp_name,
                    &sidecar_candidate_stat,
            ...
            int sidecar_rename_rc =
                w->exclusive_final_path
                    ? mira_secure_path_rename_leaf_noreplace(
                          w->parent_fd, sidecar_temp_name, sidecar_name)
                    : renameat(
                          w->parent_fd, sidecar_temp_name,
                          w->parent_fd, sidecar_name);
        ...
        int payload_rename_rc =
            w->exclusive_final_path
                ? mira_secure_path_rename_leaf_noreplace(
                      w->parent_fd, w->temp_name, w->basename)
                : renameat(
                      w->parent_fd, w->temp_name,
                      w->parent_fd, w->basename);
        if (payload_rename_rc != 0) {
            int error_number = errno;
            if (sidecar_committed && sidecar_name &&
                have_sidecar_candidate_stat) {
                unlink_leaf_if_same_identity(
                    w->parent_fd, sidecar_name,
                    &sidecar_candidate_stat);
            }
            restore_hold_entry(w->parent_fd, &sidecar_hold_name, sidecar_name);
        ...
        if (finalize_payload_hold(w->parent_fd,
                                  w->basename,
                                  w->backups,
                                  w->rotate_backups_on_commit,
                                  &payload_hold_name) != 0) {
            rc = -1;
        }
