...
static int xattr_set_string(int fd, const char *name, const char *value) {
    if (!value || value[0] == '\0') return MIRA_XATTR_OK;
...
static int xattr_apply(int fd, const char *url, const char *content_type, const char *etag,
    ...
    if (!display_url) return MIRA_XATTR_ERROR;
    ...
    for (size_t i = 0; i < sizeof(attrs) / sizeof(attrs[0]); i++) {
        int one_rc =
            xattr_set_string(fd, attrs[i].name, attrs[i].value);
        if (one_rc == MIRA_XATTR_ERROR) {
            free(display_url);
            return MIRA_XATTR_ERROR;
        }
        if (one_rc == MIRA_XATTR_UNSUPPORTED) rc = MIRA_XATTR_UNSUPPORTED;
    }
    free(display_url);
    return rc;
}
...
int mira_xattr_apply_fd(int fd, const char *url, const char *content_type,
    ...
    if (fd < 0) return MIRA_XATTR_ERROR;
    return xattr_apply(
        fd, url, content_type, etag, last_modified);
}
