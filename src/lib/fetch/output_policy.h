...
/*
 * Output policy is resolved once in struct bx_fetch_config. Runtime callers use these
...
static inline bool bx_fetch_output_is_verbose(const struct bx_fetch_config* cfg) {
    return cfg && cfg->logging.verbosity == BX_FETCH_VERBOSITY_VERBOSE;
}

static inline bool bx_fetch_output_progress_enabled(const struct bx_fetch_config* cfg) {
    return cfg && !bx_fetch_output_is_quiet(cfg) && cfg->download.show_progress;
}

static inline bool bx_fetch_output_server_response_enabled(const struct bx_fetch_config* cfg) {
    return cfg && !bx_fetch_output_is_quiet(cfg) && cfg->download.server_response;
}

static inline bool bx_fetch_output_status_enabled(const struct bx_fetch_config* cfg) {
    return cfg && !bx_fetch_output_is_quiet(cfg) && (bx_fetch_output_is_verbose(cfg) || cfg->download.server_response);
}

static inline bool bx_fetch_output_use_wget_basic(const struct bx_fetch_config* cfg) {
...
static inline bool bx_fetch_output_debug_trace_enabled(const struct bx_fetch_config* cfg) {
    return cfg && cfg->logging.debug_trace;
}

static inline bool bx_fetch_output_structured_errors_enabled(const struct bx_fetch_config* cfg) {
