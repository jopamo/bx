...
int frontier_add(Frontier* f, CrawlItem* item) {
    ...
    size_t ignored = 0;
    if (!mira_resource_bounded_strlen(item->url, MIRA_URL_MAX_BYTES, &ignored) || (item->referrer && !mira_resource_bounded_strlen(item->referrer, MIRA_URL_MAX_BYTES, &ignored))) {
        errno = EFBIG;
        crawl_item_free(item);
        return -1;
    }
...
int frontier_add_canonical(Frontier* f, CrawlItem* item) {
    ...
    if (!mira_resource_bounded_strlen(item->url, MIRA_URL_MAX_BYTES, &url_len) || (item->referrer && !mira_resource_bounded_strlen(item->referrer, MIRA_URL_MAX_BYTES, &referrer_len))) {
        errno = EFBIG;
        crawl_item_free(item);
        return -1;
    }
    ...
    if (!mira_resource_can_reserve(f->seen_count, f->retained_url_bytes, 1u, reservation_bytes, MIRA_URL_STATE_MAX_ENTRIES, MIRA_URL_STATE_MAX_BYTES)) {
        errno = EFBIG;
        crawl_item_free(item);
        return -1;
    }
...
bool frontier_is_seen(Frontier* f, const char* url) {
    ...
    size_t ignored = 0;
    if (!mira_resource_bounded_strlen(url, MIRA_URL_MAX_BYTES, &ignored)) {
        errno = EFBIG;
        return false;
    }
...
CrawlItem* crawl_item_new(const char* url, int depth, const char* referrer) {
    size_t ignored = 0;
    if ((url && !mira_resource_bounded_strlen(url, MIRA_URL_MAX_BYTES, &ignored)) || (referrer && !mira_resource_bounded_strlen(referrer, MIRA_URL_MAX_BYTES, &ignored))) {
        errno = EFBIG;
        return NULL;
    }
