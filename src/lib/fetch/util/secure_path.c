...
static int secure_path_fail_parent(int error_number, int dirfd, char *walk,
                                   char *parent, char *basename) {
    free(walk);
    close(dirfd);
    free(parent);
    free(basename);
...
static int secure_path_openat(int dirfd, const char *path, int flags,
    ...
    return (int)syscall(
        SYS_openat2, dirfd, path, &how, sizeof(how));
}
...
int mira_secure_path_rename_leaf_noreplace(int dirfd, const char *old_name,
                                           const char *new_name) {
    if (dirfd == -1 || !secure_path_is_simple_leaf(old_name) ||
        !secure_path_is_simple_leaf(new_name)) {
        errno = EINVAL;
        return -1;
    }
    return (int)syscall(
        SYS_renameat2, dirfd, old_name, dirfd, new_name,
        RENAME_NOREPLACE);
}

int mira_secure_path_check_resolution(void) {
    int fd = secure_path_openat(
        AT_FDCWD, "/", O_PATH | O_DIRECTORY | O_CLOEXEC, 0,
        RESOLVE_NO_SYMLINKS | RESOLVE_NO_XDEV);
    if (fd == -1) return -1;
...
int mira_secure_path_open_existing_file(const char *path) {
    ...
    char *basename = NULL;
    int parent_fd =
        mira_secure_path_open_parent_directory(path, false, &basename);
    if (parent_fd == -1) return -1;

    int fd = mira_secure_path_open_leaf(
        parent_fd, basename, O_RDONLY | O_CLOEXEC, 0);
...
int mira_secure_path_open_parent_directory(const char *path,
    ...
    *basename_out = NULL;
    ...
    if (parent && !(parent[0] == '/' && parent[1] == '\0')) {
        ...
        char *cursor = walk;
        if (path[0] == '/' && cursor[0] == '/') cursor++;

        char *segment = NULL;
        while ((segment = strsep(&cursor, "/")) != NULL) {
            if (segment[0] == '\0' || strcmp(segment, ".") == 0) continue;
            ...
            if (create_missing) {
                if (mkdirat(dirfd, segment, 0755) == 0) {
                    ...
                    return secure_path_fail_parent(
                        errno, dirfd, walk, parent, basename);
                }
            }

            int next_fd = secure_path_openat(
                dirfd, segment, O_RDONLY | O_DIRECTORY | O_CLOEXEC, 0,
                RESOLVE_NO_SYMLINKS);
            if (next_fd == -1) {
                return secure_path_fail_parent(
                    errno, dirfd, walk, parent, basename);
            ...
            if (created &&
                (fsync(next_fd) != 0 || fsync(dirfd) != 0)) {
                int error_number = errno;
                close(next_fd);
                return secure_path_fail_parent(
                    error_number, dirfd, walk, parent, basename);
            }
