...
static bool parse_http_date_format(const char *value, const char *format, struct tm *tm_out) {
    if (!value || !format || !tm_out) return false;
...
static void normalize_rfc850_year(struct tm *tm_value) {
    if (!tm_value) return;
    if (tm_value->tm_year < 0 || tm_value->tm_year >= 100) return;
...
bool mira_timestamp_should_use_server_time(bool no_use_server_timestamps,
    ...
    if (no_use_server_timestamps) return false;
    if (!(status == 200 || status == 206)) return false;
    if (strcmp(output_path, "-") == 0) return false;
    if (!last_modified_header || last_modified_header[0] == '\0') return false;

    time_t parsed = 0;
    if (!parse_http_date(last_modified_header, &parsed)) return false;
