    ...
    for (char *token = strtok_r(copy, ",", &saveptr);
        ...
        *end = '\0';
        ...
        if (used == capacity) {
            ...
            if (!grown) {
                for (int i = 0; i < used; i++) free(res[i]);
                free(res);
                free(copy);
                *count = -1;
                return NULL;
...
static bool host_matches_rule(const char *host, const char *rule, bool include_subdomains) {
    ...
    if (strcasecmp(host, rule) == 0) return true;
    if (!include_subdomains) return false;

    size_t host_len = strlen(host);
    size_t rule_len = strlen(rule);
    if (host_len <= rule_len) return false;
    if (host[host_len - rule_len - 1] != '.') return false;
...
static bool path_matches_directory_rule(const char *path, const char *rule) {
    ...
    size_t rule_len = strlen(rule);
    while (rule_len > 0 && rule[rule_len - 1] == '/') rule_len--;
    if (rule_len == 0) return true;
    if (strncmp(path, rule, rule_len) != 0) return false;
    return path[rule_len] == '\0' || path[rule_len] == '/';
}

static bool path_matches_any_directory(const char *path, char **rules, int count) {
    for (int i = 0; i < count; i++) {
        if (path_matches_directory_rule(path, rules[i])) return true;
    }
    return false;
}

static int append_seed_host(Filter *f, const char *host) {
    if (!f || !host || host[0] == '\0') return -1;
    if (host_matches_any(host, f->seed_hosts, f->seed_host_count, false)) return 0;
    ...
    if (!f->seed_hosts[f->seed_host_count]) return -1;
    f->seed_host_count++;
    return 0;
}

static int compile_accept_regex(Filter *f) {
    if (!f || !f->cfg || !f->cfg->recursive.accept_regex) return 0;
...
static bool url_matches_accept_regex(Filter *f, const char *url) {
    if (!f || !f->accept_regex_compiled || !url) return true;
...
Filter *filter_new(const EffectiveConfig *cfg) {
    ...
    if (f->accept_count < 0 || f->reject_count < 0 ||
        f->accept_domain_count < 0 || f->reject_domain_count < 0 ||
        f->include_dir_count < 0 || f->exclude_dir_count < 0) {
        filter_free(f);
        return NULL;
    }
...
void filter_free(Filter *f) {
    if (!f) return;
    for (int i = 0; i < f->accept_count; i++) free(f->accept_exts[i]);
    free(f->accept_exts);
    for (int i = 0; i < f->reject_count; i++) free(f->reject_exts[i]);
    free(f->reject_exts);
    for (int i = 0; i < f->accept_domain_count; i++) free(f->accept_domains[i]);
    free(f->accept_domains);
    for (int i = 0; i < f->reject_domain_count; i++) free(f->reject_domains[i]);
    free(f->reject_domains);
    for (int i = 0; i < f->include_dir_count; i++) free(f->include_dirs[i]);
    free(f->include_dirs);
    for (int i = 0; i < f->exclude_dir_count; i++) free(f->exclude_dirs[i]);
    free(f->exclude_dirs);
    for (int i = 0; i < f->seed_host_count; i++) free(f->seed_hosts[i]);
    free(f->seed_hosts);
...
static bool has_extension(const char *url, const char *ext) {
    ...
    if (slash && dot < slash) return false;
    ...
    if (strlen(ext) != ext_len) return false;

    return strncasecmp(ext_start, ext, ext_len) == 0;
}

FilterDecision filter_evaluate_url(Filter *f, const char *url) {
    if (!f) return FILTER_DECISION_ACCEPT;

    MiraProtocolDecision protocol_decision =
        mira_protocol_policy_evaluate_url(url, f->cfg->https.https_only);
    ...
    char *canonical = mira_url_canonicalize(url);
    if (!canonical) return FILTER_DECISION_INVALID_URL;
    FilterDecision decision = filter_evaluate_canonical_url(f, canonical);
    free(canonical);
    return decision;
}

static FilterDecision evaluate_transport_policy(
    const Filter *f, const MiraURL *url, const char *canonical_url) {
    if (!f || !url) return FILTER_DECISION_ACCEPT;

    MiraProtocolDecision protocol_decision =
        mira_protocol_policy_evaluate_scheme(url->scheme,
                                             f->cfg->https.https_only);
    if (protocol_decision == MIRA_PROTOCOL_DECISION_UNSUPPORTED ||
        protocol_decision == MIRA_PROTOCOL_DECISION_INVALID_URL) {
        return FILTER_DECISION_UNSUPPORTED_PROTOCOL;
    }
    if (protocol_decision == MIRA_PROTOCOL_DECISION_HTTPS_ONLY) {
        return FILTER_DECISION_HTTPS_ONLY;
    }
    if (f->cfg->http.paranoid &&
        mira_url_has_userinfo(canonical_url)) {
...
FilterDecision filter_evaluate_transport_canonical_url(
    ...
    if (!url) return FILTER_DECISION_INVALID_URL;
    FilterDecision decision =
        evaluate_transport_policy(f, url, canonical_url);
    mira_url_free(url);
    return decision;
}
...
FilterDecision filter_evaluate_canonical_url(Filter *f, const char *canonical_url) {
    ...
    MiraURL *mu = mira_url_parse(canonical_url);
    if (!mu) return FILTER_DECISION_INVALID_URL;

    FilterDecision decision =
        evaluate_transport_policy(f, mu, canonical_url);

    if (decision == FILTER_DECISION_ACCEPT) {
        const bool exact_seed_match = mu->host &&
                                      host_matches_any(mu->host, f->seed_hosts, f->seed_host_count, false);
        const bool explicit_domain_match = mu->host &&
                                           host_matches_any(mu->host, f->accept_domains,
                                                            f->accept_domain_count, true);

        if (!mu->host || mu->host[0] == '\0') {
            if (f->reject_domain_count > 0 ||
                f->accept_domain_count > 0 ||
    ...
    if (decision == FILTER_DECISION_ACCEPT) {
        ...
        } else if (f->include_dir_count > 0 &&
                   !path_matches_any_directory(mu->path, f->include_dirs,
                                               f->include_dir_count)) {
    ...
    if (decision == FILTER_DECISION_ACCEPT) {
        ...
        if (decision == FILTER_DECISION_ACCEPT && f->accept_regex_compiled &&
            !url_matches_accept_regex(f, canonical_url)) {
        ...
        if (decision == FILTER_DECISION_ACCEPT && f->accept_count > 0) {
            ...
            if (!found) decision = FILTER_DECISION_SUFFIX_ALLOWLIST;
        }
    }
