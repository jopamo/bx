...
static bool should_retry_http_status(const Scheduler *s, int status) {
    if (!s || !s->cfg->download.retry_on_http_error) return false;
...
static long long scheduler_sample_host_wait_ns(Scheduler *s) {
    if (!s || s->cfg->download.wait <= 0) return 0;
...
static bool wait_remaining_for_host(const HostState *hs, const struct timespec *now, struct timespec *remaining) {
    if (!hs || !hs->has_next_request_time) return false;
...
static bool wait_remaining_for_retry(const QueuedURL *q, const struct timespec *now, struct timespec *remaining) {
    if (!q || !q->has_retry_ready_time) return false;
...
static bool queue_wait_remaining(const Scheduler *s, const QueuedURL *q, const struct timespec *now,
    ...
    if (!host_wait && !retry_wait) return false;
...
static bool scheduler_next_wait_duration(Scheduler *s, const struct timespec *now, struct timespec *min_wait) {
    ...
    for (QueuedURL *q = s->queue_head; q; q = q->next) {
        ...
        if (hs && hs->count >= s->max_concurrent_per_host) continue;

        struct timespec remaining = {0};
        if (!queue_wait_remaining(s, q, now, &remaining)) continue;
...
static int scheduler_add_url_with_tries(Scheduler *s, const char *url, const char *output_path, int tries_done,
                                        const struct timespec *retry_ready_time) {
...
static HostState *get_host_state(const Scheduler *s, const char *host) {
    ...
    while (hs) {
        if (strcmp(hs->host, host) == 0) return hs;
        hs = hs->next;
    }
...
static bool on_transfer_complete(void *userdata, const char *url, int status, int result,
                                 bool retryable_hint) {
    TransferInfo *ti = userdata;
    Scheduler *s = ti ? ti->sched : NULL;
    if (!ti || !s) return false;

    bool retried = false;
    bool completion_ok = scheduler_require(s, s->active_total > 0,
                                           "transfer completion observed with no active transfer");
    if (completion_ok) {
        s->active_total--;
        completion_ok = scheduler_require(s, dec_host_active_count(s, ti->host),
                                          "host active-count underflow on transfer completion");
    }
    if (completion_ok) {
        completion_ok = scheduler_require(s, scheduler_counts_invariant_holds(s),
                                          "active counters diverged after transfer completion");
    }
    ...
    if (completion_ok &&
        retryable_hint &&
        should_retry_result(s, status, result) &&
        ti->tries_done < s->cfg->download.tries) {
        if (!scheduler_require(s, ti->tries_done > 0,
                               "retry candidate has invalid tries_done counter")) {
            s->had_transfer_error = true;
        }
        if (mira_output_is_verbose(s->cfg)) {
            char *display_url = mira_url_display_safe(url);
            fprintf(stderr, "  Retrying %s (try %d/%d)\n",
                    display_url ? display_url : MIRA_URL_DISPLAY_REDACTED,
                    ti->tries_done + 1,
                    s->cfg->download.tries);
            free(display_url);
        }
        ...
        if (retry_delay > 0) {
            if (!scheduler_require(s,
                                   clock_gettime(CLOCK_MONOTONIC, &retry_ready_time) == 0,
                                   "clock_gettime failed while scheduling retry delay")) {
                s->had_transfer_error = true;
            } else {
        ...
        if (!s->invariant_failed &&
            scheduler_add_url_with_tries(s, ti->url, ti->output_path, ti->tries_done, retry_ready_time_ptr) == 0) {
            retried = true;
        } else {
            s->had_transfer_error = true;
        }
...
int scheduler_add_canonical_url(Scheduler *s, const char *canonical_url,
    ...
    if (!s || !canonical_url || !output_path) return -1;
...
int scheduler_run(Scheduler *s) {
    ...
    while (s->queue_head || s->active_total > 0) {
        if (!scheduler_require(s, scheduler_counts_invariant_holds(s),
                               "active counters diverged at scheduler loop entry")) {
            return -1;
        }
        ...
        if (!scheduler_require(s,
                               ...
                               "clock_gettime failed while evaluating scheduler timing")) {
            return -1;
        }
        ...
        while (q && s->active_total < s->max_concurrent_global) {
            ...
            if (host_active < s->max_concurrent_per_host && wait_ok) {
                if (!scheduler_require(s,
                                       q->tries_done >= 0,
                                       "queued transfer has negative tries_done")) {
                    return -1;
                }
                if (s->cfg->download.tries > 0 &&
                    !scheduler_require(s,
                                       q->tries_done < s->cfg->download.tries,
                                       "queued transfer exceeded retry budget before dispatch")) {
                    return -1;
                }
                if (q->has_retry_ready_time &&
                    !scheduler_require(s,
                                       !wait_remaining_for_retry(q, &now, NULL),
                                       "retry dispatched before retry-ready time elapsed")) {
                    return -1;
                }
                ...
                if (dispatch_rc == 0) {
                    ...
                    started_any = true;
                    if (!host_count_ok) {
                        scheduler_record_invariant_failure(
                            s, "failed to update per-host active counters after dispatch");
                        return -1;
                    }
                    if (!scheduler_require(s,
                                           scheduler_counts_invariant_holds(s),
                                           "active counters diverged after dispatch")) {
                        return -1;
                    }
                    continue;
                }
        ...
        if (s->active_total > 0) {
            if (!scheduler_require(s, scheduler_counts_invariant_holds(s),
                                   "active counters diverged before scheduler poll")) {
                return -1;
            }
            if (!s->poll || s->poll(s->userdata) != 0) return -1;
        } else if (!started_any && s->queue_head) {
            struct timespec wait_duration = {0};
            if (scheduler_next_wait_duration(s, &now, &wait_duration)) {
                if (scheduler_sleep(&wait_duration) != 0) return -1;
                continue;
            }
            break;
        }
    ...
    if (!scheduler_require(s, scheduler_counts_invariant_holds(s),
                           "active counters diverged at scheduler loop exit")) {
        return -1;
    }
