...
static bool span_equals_case(const char *value, size_t value_len,
                             const char *expected) {
    if (!value || !expected || strlen(expected) != value_len) return false;
    for (size_t i = 0; i < value_len; i++) {
        if (ascii_lower((unsigned char)value[i]) !=
...
static bool parse_token_span(const char **cursor, const char **start_out,
                             size_t *length_out) {
    const char *start = *cursor;
    while (ascii_is_tchar((unsigned char)**cursor)) (*cursor)++;
...
static bool parse_parameter_value(const char **cursor,
                                  HeaderParameterValue *value_out) {
    if (**cursor != '"') {
        value_out->quoted = false;
        return parse_token_span(
            cursor, &value_out->start, &value_out->length);
    }
...
static bool span_is_utf8(const unsigned char *value, size_t length) {
    ...
    while (i < length) {
        ...
        if (continuation_count > length - i ||
            value[i] < second_min || value[i] > second_max) {
            return false;
        }
        i++;
        for (size_t j = 1; j < continuation_count; j++, i++) {
            if (value[i] < 0x80 || value[i] > 0xbf) return false;
        }
    }
    return true;
}
...
static char *decode_extended_filename(const HeaderParameterValue *value) {
    ...
    const char *second_quote =
        memchr(first_quote + 1, '\'',
               (size_t)(end - (first_quote + 1)));
    if (!second_quote ||
        !span_equals_case(
            value->start, (size_t)(first_quote - value->start), "UTF-8")) {
        errno = EINVAL;
        return NULL;
    }
    ...
    if (!span_has_safe_filename_bytes(decoded, out) ||
        !span_is_utf8((const unsigned char *)decoded, out)) {
        free(decoded);
        errno = EINVAL;
        return NULL;
    }
...
static bool framing_name_is_forbidden(const char *name, size_t name_len) {
    return span_equals_case(name, name_len, "Content-Length") ||
           span_equals_case(name, name_len, "Transfer-Encoding") ||
           span_equals_case(name, name_len, "Trailer");
}

static MiraHttpHeaderError validate_name(const char *name, size_t name_len) {
    if (!name || name_len == 0) return MIRA_HTTP_HEADER_INVALID_NAME;
    for (size_t i = 0; i < name_len; i++) {
        if (!ascii_is_tchar((unsigned char)name[i])) {
            return MIRA_HTTP_HEADER_INVALID_NAME;
        }
    }
    if (framing_name_is_forbidden(name, name_len)) {
        return MIRA_HTTP_HEADER_FORBIDDEN_FRAMING;
    }
    return MIRA_HTTP_HEADER_OK;
}

static MiraHttpHeaderError normalize_value_span(
    const char *value, size_t value_len,
    const char **trimmed_out, size_t *trimmed_len_out) {
    if (!value || !trimmed_out || !trimmed_len_out) {
        return MIRA_HTTP_HEADER_INVALID_ARGUMENT;
    }

    for (size_t i = 0; i < value_len; i++) {
        unsigned char c = (unsigned char)value[i];
        if ((c < 0x20 && c != '\t') || c == 0x7f) {
            return MIRA_HTTP_HEADER_INVALID_VALUE;
        }
    }
...
static MiraHttpHeaderError parse_line(const char *line,
                                      MiraHttpHeaderView *view) {
    if (!line || !view) return MIRA_HTTP_HEADER_INVALID_ARGUMENT;

    const char *colon = strchr(line, ':');
    if (!colon) return MIRA_HTTP_HEADER_MALFORMED_LINE;

    size_t name_len = (size_t)(colon - line);
    MiraHttpHeaderError error = validate_name(line, name_len);
    if (error != MIRA_HTTP_HEADER_OK) return error;

    const char *value = colon + 1;
    const char *trimmed = NULL;
    size_t trimmed_len = 0;
    error = normalize_value_span(value, strlen(value),
                                 &trimmed, &trimmed_len);
    if (error != MIRA_HTTP_HEADER_OK) return error;
...
static MiraHttpHeaderError view_pair(const char *name, const char *value,
                                     MiraHttpHeaderView *view) {
    if (!name || !value || !view) {
        return MIRA_HTTP_HEADER_INVALID_ARGUMENT;
    }
...
static MiraHttpHeaderError format_view(const MiraHttpHeaderView *view,
                                       bool curl_format,
                                       char **formatted_out) {
    ...
    *formatted_out = NULL;
    ...
    if (view->name_len > SIZE_MAX - separator_len ||
        view->name_len + separator_len > SIZE_MAX - view->value_len) {
        return MIRA_HTTP_HEADER_OUT_OF_MEMORY;
    }
    size_t formatted_len =
        view->name_len + separator_len + view->value_len;
    if (formatted_len == SIZE_MAX) return MIRA_HTTP_HEADER_OUT_OF_MEMORY;

    char *formatted = calloc(formatted_len + 1, 1);
    if (!formatted) return MIRA_HTTP_HEADER_OUT_OF_MEMORY;
    ...
    if (view->value_len == 0) {
        ...
        memcpy(formatted + view->name_len + 2,
               view->value, view->value_len);
    }
...
MiraHttpHeaderError mira_http_header_normalize_pair(
    ...
    if (normalized_name_out) *normalized_name_out = NULL;
    if (normalized_value_out) *normalized_value_out = NULL;
    ...
    if (!normalized_name) return MIRA_HTTP_HEADER_OUT_OF_MEMORY;
...
MiraHttpHeaderError mira_http_header_format_line_for_curl(
    ...
    if (formatted_out) *formatted_out = NULL;
    MiraHttpHeaderView view;
    MiraHttpHeaderError error = parse_line(line, &view);
    if (error != MIRA_HTTP_HEADER_OK) return error;
...
MiraContentDispositionResult mira_http_content_disposition_filename(
    ...
    if (filename_out) *filename_out = NULL;
    if (!value || !filename_out) {
        return MIRA_CONTENT_DISPOSITION_INVALID;
    }
    ...
    if (!parse_token_span(
            &cursor, &disposition, &disposition_length)) {
        return MIRA_CONTENT_DISPOSITION_INVALID;
    }
    (void)disposition;
    ...
    while (*cursor != '\0') {
        ...
        if (!parse_token_span(&cursor, &name, &name_length)) goto invalid;
        skip_ows(&cursor);
        if (*cursor != '=') goto invalid;
        cursor++;
        skip_ows(&cursor);
        ...
        if (!parse_parameter_value(&cursor, &parameter_value)) goto invalid;
        skip_ows(&cursor);
        if (*cursor != '\0' && *cursor != ';') goto invalid;

        bool is_filename =
            span_equals_case(name, name_length, "filename");
        bool is_extended_filename =
            span_equals_case(name, name_length, "filename*");
        if (!is_filename && !is_extended_filename) continue;

        if ((is_filename && saw_filename) ||
            (is_extended_filename && saw_extended_filename)) {
            goto invalid;
        }

        errno = 0;
        char *decoded =
            is_extended_filename
                ? decode_extended_filename(&parameter_value)
                : decode_parameter_value(&parameter_value);
        if (!decoded) {
            if (errno == ENOMEM) goto out_of_memory;
            goto invalid;
        }
    ...
    if (extended_filename) {
        ...
        *filename_out = extended_filename;
        return MIRA_CONTENT_DISPOSITION_FILENAME;
    }
    if (filename) {
        *filename_out = filename;
        return MIRA_CONTENT_DISPOSITION_FILENAME;
    }
    return MIRA_CONTENT_DISPOSITION_NONE;

out_of_memory:
    free(filename);
    free(extended_filename);
    return MIRA_CONTENT_DISPOSITION_OUT_OF_MEMORY;
