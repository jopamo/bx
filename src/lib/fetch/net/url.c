...
static MiraProtocol protocol_from_scheme_span(const char *scheme, size_t scheme_len) {
    if (!scheme || scheme_len == 0) return MIRA_PROTOCOL_NONE;

    for (size_t i = 0;
         i < sizeof(k_supported_protocols) / sizeof(k_supported_protocols[0]);
         i++) {
        if (strlen(k_supported_protocols[i].scheme) == scheme_len &&
            strncasecmp(scheme, k_supported_protocols[i].scheme, scheme_len) == 0) {
...
static bool url_scheme_span(const char *url, const char **scheme, size_t *scheme_len) {
    if (!url || !scheme || !scheme_len ||
        !isalpha((unsigned char)url[0])) {
        return false;
    }
...
bool mira_url_has_scheme(const char *url, const char *expected_scheme) {
    if (!url || !expected_scheme) return false;

    const char *scheme = NULL;
    size_t scheme_len = 0;
    if (!url_scheme_span(url, &scheme, &scheme_len) ||
        strlen(expected_scheme) != scheme_len ||
        strncasecmp(scheme, expected_scheme, scheme_len) != 0) {
        return false;
    }
...
bool mira_url_has_userinfo(const char *url) {
    const char *scheme = NULL;
    size_t scheme_len = 0;
    if (!url_scheme_span(url, &scheme, &scheme_len)) return false;

    const char *authority = scheme + scheme_len + 1;
    if (authority[0] != '/' || authority[1] != '/') return false;
    authority += 2;
...
static bool url_has_valid_percent_escapes(const char *url) {
    ...
    for (size_t i = 0; url[i] != '\0'; i++) {
        if (url[i] != '%') continue;
        if (url[i + 1] == '\0' || url[i + 2] == '\0') return false;
        if (!ascii_is_hex((unsigned char)url[i + 1]) ||
            !ascii_is_hex((unsigned char)url[i + 2])) {
            return false;
        }
        i += 2;
    }
...
static bool uri_component_allows(URLComponentKind kind, unsigned char c) {
    if (uri_is_unreserved(c) || uri_is_subdelimiter(c)) return true;
    if (kind == URL_COMPONENT_USERINFO) return c == ':';
    if (kind == URL_COMPONENT_PATH) return c == ':' || c == '@' || c == '/';
...
static char *canonicalize_uri_component(const char *value, URLComponentKind kind) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (c == '%') {
            if (i + 2 >= len ||
                !ascii_is_hex((unsigned char)value[i + 1]) ||
                !ascii_is_hex((unsigned char)value[i + 2])) {
                free(canonical);
                errno = EINVAL;
                return NULL;
            }
            ...
            unsigned char decoded =
                (unsigned char)((ascii_hex_value((unsigned char)value[i + 1]) << 4U) |
                                ascii_hex_value((unsigned char)value[i + 2]));
            canonical[out++] = '%';
...
MiraProtocolDecision mira_protocol_policy_evaluate_url(const char *url,
    ...
    if (!parsed) return MIRA_PROTOCOL_DECISION_INVALID_URL;

    MiraProtocolDecision decision =
        mira_protocol_policy_evaluate_scheme(parsed->scheme, https_only);
...
static bool is_default_port(const char *scheme, const char *port) {
    if (!scheme || !port || port[0] == '\0') return false;
    if (strcasecmp(scheme, "http") == 0) return strcmp(port, "80") == 0;
    if (strcasecmp(scheme, "https") == 0) return strcmp(port, "443") == 0;
    if (strcasecmp(scheme, "ftp") == 0) return strcmp(port, "21") == 0;
    if (strcasecmp(scheme, "ftps") == 0) return strcmp(port, "990") == 0;
    return false;
}
...
MiraURL *mira_url_parse(const char *url) {
    ...
    char *s;
    if ((s = mu->scheme)) { mu->scheme = strdup(s); curl_free(s); }
    if ((s = mu->user)) { mu->user = strdup(s); curl_free(s); }
    if ((s = mu->password)) { mu->password = strdup(s); curl_free(s); }
    if ((s = mu->host)) { mu->host = strdup(s); curl_free(s); }
    if ((s = mu->path)) { mu->path = strdup(s); curl_free(s); }
    if ((s = mu->query)) { mu->query = strdup(s); curl_free(s); }
    if ((s = mu->fragment)) { mu->fragment = strdup(s); curl_free(s); }
...
char *mira_url_to_string(MiraURL *mu) {
    ...
    if (mu->scheme) curl_url_set(h, CURLUPART_SCHEME, mu->scheme, 0);
    if (mu->user) curl_url_set(h, CURLUPART_USER, mu->user, 0);
    if (mu->password) curl_url_set(h, CURLUPART_PASSWORD, mu->password, 0);
    if (mu->host) curl_url_set(h, CURLUPART_HOST, mu->host, 0);
    if (mu->port > 0) {
        char port_str[16];
        snprintf(port_str, sizeof(port_str), "%d", mu->port);
        curl_url_set(h, CURLUPART_PORT, port_str, 0);
    }
    if (mu->path) curl_url_set(h, CURLUPART_PATH, mu->path, 0);
    if (mu->query) curl_url_set(h, CURLUPART_QUERY, mu->query, 0);
    if (mu->fragment) curl_url_set(h, CURLUPART_FRAGMENT, mu->fragment, 0);
...
static char *canonicalize_url(const char *url, bool include_userinfo) {
    errno = 0;
    if (!url || !url_has_explicit_authority(url) ||
    ...
    if (curl_url_get(src, CURLUPART_SCHEME, &scheme, 0) == CURLUE_OK && scheme) {
        ...
        if (curl_url_set(dst, CURLUPART_SCHEME, scheme, 0) != CURLUE_OK) goto cleanup;
    } else {
        errno = EINVAL;
        goto cleanup;
    }
    ...
    if (host_rc == CURLUE_OK && host && host[0] != '\0' && string_is_ascii(host)) {
        ...
        if (curl_url_set(dst, CURLUPART_HOST, host, 0) != CURLUE_OK) goto cleanup;
    } else {
        errno = (host_rc == CURLUE_LACKS_IDN) ? ENOTSUP : EINVAL;
        goto cleanup;
    }
    if (include_userinfo &&
        curl_url_get(src, CURLUPART_USER, &user, 0) == CURLUE_OK && user) {
        canonical_user = canonicalize_uri_component(user, URL_COMPONENT_USERINFO);
        if (!canonical_user ||
            curl_url_set(dst, CURLUPART_USER, canonical_user, 0) != CURLUE_OK) {
            goto cleanup;
        }
    }
    if (include_userinfo &&
        curl_url_get(src, CURLUPART_PASSWORD, &password, 0) == CURLUE_OK &&
        password) {
        canonical_password = canonicalize_uri_component(password, URL_COMPONENT_USERINFO);
        if (!canonical_password ||
            curl_url_set(dst, CURLUPART_PASSWORD, canonical_password, 0) != CURLUE_OK) {
            goto cleanup;
        }
    }
    if (curl_url_get(src, CURLUPART_PATH, &path, 0) == CURLUE_OK && path && path[0] != '\0') {
        ...
        if (!canonical_path ||
            curl_url_set(dst, CURLUPART_PATH, canonical_path, 0) != CURLUE_OK) {
            goto cleanup;
        }
    ...
    if (query_rc == CURLUE_OK && query) {
        ...
        if (!canonical_query ||
            curl_url_set(dst, CURLUPART_QUERY, canonical_query, 0) != CURLUE_OK) {
            goto cleanup;
        }
    }
    if (curl_url_get(src, CURLUPART_PORT, &port, 0) == CURLUE_OK && port && port[0] != '\0') {
        if (!is_default_port(scheme, port)) {
            if (curl_url_set(dst, CURLUPART_PORT, port, 0) != CURLUE_OK) goto cleanup;
        }
    }
    if (curl_url_get(src, CURLUPART_ZONEID, &zone_id, 0) == CURLUE_OK && zone_id) {
        ...
        if (!canonical_zone_id ||
            curl_url_set(dst, CURLUPART_ZONEID, canonical_zone_id, 0) != CURLUE_OK) {
            goto cleanup;
        }
    }
...
char *mira_url_display_safe(const char *url) {
    ...
    if (!canonical) {
        ...
        size_t scheme_len = 0;
        if (!url_scheme_span(url, &scheme, &scheme_len) ||
            protocol_from_scheme_span(scheme, scheme_len) != MIRA_PROTOCOL_NONE ||
            !url_has_explicit_authority(url) ||
            !url_has_valid_percent_escapes(url)) {
            return NULL;
        }
        ...
        char *display = NULL;
        if (curl_url_set(unsupported, CURLUPART_URL, url,
                         CURLU_NON_SUPPORT_SCHEME) == CURLUE_OK &&
            curl_url_set(unsupported, CURLUPART_USER, NULL, 0) == CURLUE_OK &&
            curl_url_set(unsupported, CURLUPART_PASSWORD, NULL, 0) == CURLUE_OK &&
            curl_url_set(unsupported, CURLUPART_FRAGMENT, NULL, 0) == CURLUE_OK &&
            curl_url_get(unsupported, CURLUPART_URL, &display,
                         CURLU_GET_EMPTY) == CURLUE_OK &&
            display) {
