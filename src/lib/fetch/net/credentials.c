...
void net_select_origin_credentials(const EffectiveConfig* cfg, const char* url, MiraCredentialSelection* selection) {
    ...
    *selection = (MiraCredentialSelection){0};
    ...
    bool is_http = mira_url_has_scheme(url, "http") || mira_url_has_scheme(url, "https");
    bool is_ftp = mira_url_has_scheme(url, "ftp") || mira_url_has_scheme(url, "ftps");

    if (is_http && pair_is_configured(cfg->http.http_user, cfg->http.http_password)) {
        select_configured_pair(selection, MIRA_CREDENTIAL_SOURCE_HTTP, cfg->http.http_user, cfg->http.http_password);
        return;
    }
    if (is_ftp && pair_is_configured(cfg->ftp.ftp_user, cfg->ftp.ftp_password)) {
        select_configured_pair(selection, MIRA_CREDENTIAL_SOURCE_FTP, cfg->ftp.ftp_user, cfg->ftp.ftp_password);
        return;
    }
    if (pair_is_configured(cfg->download.user, cfg->download.password)) {
        select_configured_pair(selection, MIRA_CREDENTIAL_SOURCE_GENERIC, cfg->download.user, cfg->download.password);
        return;
    }
...
void net_select_proxy_credentials(const EffectiveConfig* cfg, MiraCredentialSelection* selection) {
    ...
    *selection = (MiraCredentialSelection){
    ...
    if (pair_is_configured(cfg->http.proxy_user, cfg->http.proxy_password)) {
        select_configured_pair(selection, MIRA_CREDENTIAL_SOURCE_PROXY_CONFIG, cfg->http.proxy_user, cfg->http.proxy_password);
    }
}
