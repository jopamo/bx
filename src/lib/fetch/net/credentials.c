...
static void select_configured_pair(MiraCredentialSelection *selection,
...
void net_select_origin_credentials(const EffectiveConfig *cfg,
    ...
    *selection = (MiraCredentialSelection){0};
    ...
    bool is_http = mira_url_has_scheme(url, "http") ||
                   mira_url_has_scheme(url, "https");
    bool is_ftp = mira_url_has_scheme(url, "ftp") ||
                  mira_url_has_scheme(url, "ftps");

    if (is_http &&
        pair_is_configured(cfg->http.http_user,
                           cfg->http.http_password)) {
        select_configured_pair(selection,
    ...
    if (is_ftp &&
        pair_is_configured(cfg->ftp.ftp_user,
                           cfg->ftp.ftp_password)) {
        select_configured_pair(selection,
    ...
    if (pair_is_configured(cfg->download.user,
                           cfg->download.password)) {
        select_configured_pair(selection,
    ...
    if (mira_url_has_userinfo(url)) {
        selection->source = MIRA_CREDENTIAL_SOURCE_URL;
    }
}
...
void net_select_proxy_credentials(const EffectiveConfig *cfg,
    ...
    *selection = (MiraCredentialSelection){
        .source = MIRA_CREDENTIAL_SOURCE_PROXY_INHERITED,
    };
    if (!cfg) return;

    if (pair_is_configured(cfg->http.proxy_user,
                           cfg->http.proxy_password)) {
        select_configured_pair(selection,
...
static const char *first_environment_value(const char *lower_name,
...
static bool proxy_authority_has_userinfo(const char *proxy_url) {
    ...
    if (scheme_separator) {
        return mira_url_has_userinfo(proxy_url);
    }
...
int net_sanitize_proxy_url_for_explicit_credentials(
    ...
    if (!sanitized_out) return -1;
    *sanitized_out = NULL;
    if (!proxy_authority_has_userinfo(proxy_url)) return 0;
    ...
    if (!strstr(proxy_url, "://")) {
        ...
        if (proxy_len > SIZE_MAX - sizeof(prefix)) return -1;
        owned_parse_url = malloc(sizeof(prefix) - 1 + proxy_len + 1);
        if (!owned_parse_url) return -1;
        memcpy(owned_parse_url, prefix, sizeof(prefix) - 1);
        memcpy(owned_parse_url + sizeof(prefix) - 1,
               proxy_url, proxy_len + 1);
        parse_url = owned_parse_url;
    }

    char *sanitized = mira_url_display_safe(parse_url);
