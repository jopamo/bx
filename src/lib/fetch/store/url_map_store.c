...
static int url_map_store_fail_tmp_file(int error_number, int dirfd, int fd,
                                       char **tmp_name_inout) {
    if (fd != -1) close(fd);
...
static char *output_document_dir(const EffectiveConfig *cfg) {
    ...
    if (last_slash == output_document) return strdup("/");
...
static char *store_path_for(const EffectiveConfig *cfg) {
    ...
    char *path = NULL;
    if (asprintf(&path, "%s/%s%016llx", store_dir, URL_MAP_STORE_FILE_PREFIX,
                 (unsigned long long)scope_hash) == -1) {
        free(store_dir);
        return NULL;
    }
...
static int hex_encode(const char *input, char **encoded_out) {
    if (!input || !encoded_out) return -1;

    size_t len = strlen(input);
    if (len > (SIZE_MAX - 1U) / 2U) return -1;
...
static int hex_decode(const char *encoded, char **decoded_out) {
    ...
    if ((len % 2U) != 0U ||
        len / 2u > MIRA_URL_MAP_MAX_FIELD_BYTES) {
        errno = len / 2u > MIRA_URL_MAP_MAX_FIELD_BYTES
                    ? EFBIG
                    : EINVAL;
        return -1;
    }
...
static int open_store_dir(const char *path, int *dirfd_out, char **basename_out) {
    ...
    char *basename = NULL;
    int dirfd =
        mira_secure_path_open_parent_directory(path, true, &basename);
    if (dirfd == -1) return -1;
...
static int cleanup_orphan_temp_files(int dirfd, const char *basename, bool *removed_any) {
    ...
    free(prefix);
    if (rc != 0) return -1;
    if (closed_rc != 0) return -1;
    return 0;
}

static FILE *open_unique_tmp_file(int dirfd, const char *basename,
                                  char **tmp_name_out) {
    if (dirfd == -1 || !basename || !tmp_name_out) return NULL;
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        ...
        char *tmp_name = NULL;
        if (asprintf(&tmp_name, "%s.tmp.%ld.%llu.%u",
                     basename, (long)getpid(), sequence, attempt) == -1) {
            return NULL;
        }
        ...
        if (fd == -1) {
            ...
            free(tmp_name);
            if (error_number == EEXIST) continue;
            errno = error_number;
            return NULL;
        }
        FILE *f = fdopen(fd, "w");
 ...
 */
static StoreLineResult read_store_line(
    FILE *f, char *line, size_t capacity, size_t *store_bytes) {
    if (!f || !line || !store_bytes ||
        capacity < MIRA_URL_MAP_ENCODED_LINE_MAX_BYTES + 1u) {
        errno = EINVAL;
    ...
    for (;;) {
        ...
        if (byte == EOF) {
            ...
            if (length == 0) return STORE_LINE_EOF;
            break;
        }

        if (*store_bytes >= MIRA_URL_MAP_STORE_MAX_BYTES ||
            length >= MIRA_URL_MAP_ENCODED_LINE_MAX_BYTES) {
            errno = EFBIG;
            return STORE_LINE_ERROR;
        }
        (*store_bytes)++;
...
int url_map_store_load(const EffectiveConfig *cfg, MiraUrlMapLoadFn cb, void *userdata) {
    ...
    int store_fd = mira_secure_path_open_leaf(
        dirfd, basename, O_RDONLY | O_CLOEXEC | O_NOFOLLOW, 0);
    ...
    while (true) {
        if (rc != 0) break;
        StoreLineResult line_result =
            read_store_line(
                f, line,
                MIRA_URL_MAP_ENCODED_LINE_MAX_BYTES + 1u,
                &store_bytes);
        if (line_result == STORE_LINE_EOF) break;
        ...
        *tab = '\0';
        ...
        if (url_len > SIZE_MAX - path_len ||
            !mira_resource_can_reserve(
                entry_count, decoded_bytes,
                1u, url_len + path_len,
                MIRA_URL_MAP_MAX_ENTRIES,
                MIRA_URL_MAP_MAX_DECODED_BYTES)) {
            free(url);
            free(local_path);
            failure_errno = EFBIG;
            rc = -1;
    ...
    if (rc != 0) errno = failure_errno ? failure_errno : EIO;
    return rc;
}
...
int url_map_store_save(const EffectiveConfig *cfg, const MiraUrlMapEntry *entries, size_t entry_count) {
    ...
    for (size_t i = 0; i < entry_count; i++) {
        ...
        if (!mira_resource_bounded_strlen(
                entries[i].url, MIRA_URL_MAP_MAX_FIELD_BYTES,
                &source_url_len) ||
            !mira_resource_bounded_strlen(
                entries[i].local_path, MIRA_URL_MAP_MAX_FIELD_BYTES,
                &path_len)) {
            entry_refs_free(sorted, entry_count);
            errno = EFBIG;
            return -1;
        ...
        if (!mira_resource_bounded_strlen(
                sorted[i].url, MIRA_URL_MAP_MAX_FIELD_BYTES,
                &public_url_len) ||
            public_url_len > SIZE_MAX - path_len ||
            !mira_resource_can_reserve(
                i, decoded_bytes, 1u, public_url_len + path_len,
                MIRA_URL_MAP_MAX_ENTRIES,
                MIRA_URL_MAP_MAX_DECODED_BYTES)) {
            entry_refs_free(sorted, entry_count);
            errno = EFBIG;
    ...
    if (rc == 0 && flush_and_sync_stream(f) != 0) rc = -1;
    if (fclose(f) != 0) rc = -1;

    if (rc == 0 &&
        renameat(dirfd, tmp_name, dirfd, basename) != 0) {
        rc = -1;
    }
    if (rc == 0 && fsync(dirfd) != 0) rc = -1;
    if (rc != 0) unlinkat(dirfd, tmp_name, 0);
