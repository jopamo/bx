        ...
        if (ctx->output_to_stdout) {
            ...
            if (lstat(out_path, &st) == 0) {
                if (S_ISLNK(st.st_mode)) {
                    zu_context_set_error(ctx, ZU_STATUS_IO, "refusing to overwrite symlink");
                    rc = ZU_STATUS_IO;
                    goto cleanup;
                }
        ...
        if (zrc != Z_OK) {
            ...
            while (rc == ZU_STATUS_OK && remaining > 0) {
                ...
                do {
                    ...
                    /* A compressed input chunk can end between DEFLATE symbols. */
                    bool input_boundary = zrc == Z_BUF_ERROR && strm.avail_in == 0 && remaining > 0;
                    if (zrc != Z_OK && zrc != Z_STREAM_END && !input_boundary) {
                        rc = ZU_STATUS_IO;
            ...
            if (rc == ZU_STATUS_OK) {
                if (zrc == Z_STREAM_END && remaining > 0) {
                    /*
                     * fread already advanced past strm.avail_in.  Only bytes not
                     * yet read from the declared compressed payload remain.
                     */
