...
static int md5sum_hash_path(const char* path, uint8_t out[BX_MD5_DIGEST_SIZE]) {
    ...
    return bx_digest_file(&ctx, sizeof(ctx), md5_init_adapter, md5_update_adapter, md5_final_adapter, path, out, BX_MD5_DIGEST_SIZE);
}
...
static int md5sum_verify_stream(FILE* stream, const char* source_name, const struct md5sum_options* options) {
    char* line = NULL;
    size_t cap = 0u;
    ...
    while (true) {
        ...
        if (md5sum_hash_path(expected.filename, actual) != 0) {
            if (!options->status) {
                fprintf(stderr, "%s: %s: %s\n", options->progname, expected.filename, strerror(errno));
            }
            exit_code = MD5SUM_EXIT_FAIL;
            continue;
        }
